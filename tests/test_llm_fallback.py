...
async def test_extract_with_llm_returns_validated_records(monkeypatch):
    ...
    mock_call = AsyncMock(
        return_value=LlmExtractionPayload(
            records=[
                LlmPaymentRecord(
                    record_type="RECEIPT",
                    payee="Example SaaS",
...
async def test_extract_with_llm_rejects_hallucinated_quote(monkeypatch):
    ...
    with pytest.raises(ExtractionRejected):
        await extract_with_llm(msg, settings)

    assert mock_call.await_count == 2  # citation_max_retries=1 → 2 attempts
...
async def test_run_cascade_uses_llm_after_template_miss(monkeypatch):
    ...
    mock_extract = AsyncMock(return_value=[llm_record])
...
async def test_run_cascade_skips_llm_without_api_key():
    ...
    records, _extractor_name, template_miss = await run_cascade(msg, settings)
